...
| `_COPY_PARENT_GEOM` | If present in the feature type (there is no actual value needed, the attribute just needs to be in the feature type), then if a newly created feature has a parent and they are both points then the child will copy the parent geometry. | `` |
