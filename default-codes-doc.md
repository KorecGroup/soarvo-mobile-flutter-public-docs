...
* **dbQuerySingleValue(query, dbName)** — Run a read-only query against a location database file and return a single value. The query must return exactly one row and one column.
