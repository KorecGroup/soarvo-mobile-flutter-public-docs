...
| Code | Description | Example | Tags |
|------|------------|---------|------|
| !LOADIMAGE(filename) | Loads a specific image file directly into the sketch canvas | !SKETCHLOAD(photoname.png) | sketch, image, load |
| !LOADIMAGE([attributename].png) | Loads an image linked to an attribute value, where the attribute name in square brackets is replaced with the actual attribute value | !SKETCHLOAD([inspection_id].png) | sketch, image, load, dynamic |
